...
test("enabling compression engages a real Web Audio graph on the video", async ({ page, serviceWorker }) => {
...
test("the meter stays live (transparent graph) even with compression off", async ({ page, serviceWorker }) => {
  await setStorage(serviceWorker, { audioComp: false });
  await page.goto("/");
