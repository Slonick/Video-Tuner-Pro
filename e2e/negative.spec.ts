...
test("ignores keyboard shortcuts while typing in a field", async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    i.id = "f"; document.body.appendChild(i); i.focus();
  });
...
test("clamps an out-of-range speed instead of applying it verbatim", async ({ page, serviceWorker }) => {
  await page.goto("/");
  ...
  const lo = await sendToContent(serviceWorker, "setSpeed", { speed: 0.001 }) as { speed: number };
...
test("a live stream refuses a manual speed change", async ({ page, serviceWorker }) => {
  ...
  const resp = await sendToContent(serviceWorker, "setSpeed", { speed: 1.5 }) as { speed: number; live: boolean };
  expect(resp.live).toBe(true);
  expect(resp.speed).not.toBeCloseTo(1.5, 2); // manual change refused — live is owned by sync
});

test("the badge does NOT appear when the on-video display is off", async ({ page, serviceWorker }) => {
