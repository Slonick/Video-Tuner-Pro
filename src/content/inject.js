...
(function () {
  ...
  // --- Twitch ---------------------------------------------------------------
  // Pull the live latency (seconds) from a Twitch player instance. Exposed two
  // ways across versions: getLiveLatency() and the statistics object's
  // hlsLatencyBroadcaster (what the "Latency To Broadcaster" row reads).
  function twitchLatencyOf(pl) {
    try {
  ...
  // --- YouTube --------------------------------------------------------------
  // YouTube's #movie_player exposes "Live Latency" via getStatsForNerds(); a
  // live stream's distance behind the seekable edge is the same thing as a
  // fallback (guarded so it never fires on VODs).
  function youtubeLatency() {
    try {
      const yp = document.getElementById("movie_player") || document.querySelector(".html5-video-player");
      if (!yp) return null;
