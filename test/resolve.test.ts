...
describe("resolveSyncTarget", () => {
  ...
  it("a channel target wins over site + global", () => {
    expect(resolveSyncTarget(["twitch:shroud"], D, { [D]: 8 }, { "twitch:shroud": 3 }, 12))
      .toEqual({ target: 3, scope: "channel" });
  });

  it("matches a channel target saved under either key form", () => {
    expect(resolveSyncTarget(["UC1", "@h"], "youtube.com", {}, { "@h": 6 }, undefined))
      .toEqual({ target: 6, scope: "channel" });
  });

  it("falls to the site target when no channel target", () => {
    expect(resolveSyncTarget(["twitch:x"], D, { [D]: 8 }, {}, 12))
      .toEqual({ target: 8, scope: "site" });
  });

  it("falls to the global target when no channel/site target", () => {
    expect(resolveSyncTarget([], D, {}, {}, 12))
      .toEqual({ target: 12, scope: "global" });
  });

  it("falls to the 5s default with nothing saved (scope null)", () => {
    expect(resolveSyncTarget([], D, {}, {}, undefined))
      .toEqual({ target: 5, scope: null });
  });
});
